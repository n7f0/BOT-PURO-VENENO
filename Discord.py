...
class FarmProdutosModal(Modal, title="Registrar Farm Produtos"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await log_acao("registrar_farm", interaction.user, f"Produtos: {', '.join(f'{p[\"produto\"]}: {p[\"quantidade\"]}' for p in produtos)}")
        await log_admin("NOVA FARM PRODUTOS", f"Usuário: {interaction.user.mention}\nProdutos: {', '.join(f'{p[\"produto\"]}: {p[\"quantidade\"]}' for p in produtos)}", 0x00ff00)
        await atualizar_ranking()
